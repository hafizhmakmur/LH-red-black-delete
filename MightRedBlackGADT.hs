...
{-@ measure isEmpty @-}
isEmpty :: CT n c a -> Bool
isEmpty E = True
isEmpty _ = False

-- maximum element from a red-black tree
{-@ max :: {x:CT n c a | not isEmpty x} -> a @-}
max :: CT n c a -> a
...
{-@ removeMax :: {x:CT n c a | not isEmpty x} -> DT n a @-}
...
{-
{-@ measure sColorEq @-}
sColorEq :: SColor a -> SColor b -> Bool
sColorEq c1 c2 = case testEquality c1 c2 of
    Just Refl -> True
    Nothing -> False

--{-@ assume isJust :: Maybe a -> Bool @-}
{-@ measure proveRed @-}
proveRed :: DT n a -> Bool
proveRed (DT c1 (T c2 aa ax ay) x (T c3 ac az ad)) =  
    sColorEq c1 R &&
    sColorEq c2 B &&
    sColorEq c3 B
--proveRed _ = False
...
dbalanceL c a x b = undefined --error ("no case for " ++ showDT a)
...
dbalanceR c a x b = undefined --error ("no case for " ++ showDT b)
...
{-@ prop_delete_bst :: RBSet Int -> True @-}
