...
bbb (T _ E _ _) = E
bbb (T _ _ _ E) = E
bbb t@(T c l x r) = ddd c l (eee r)
 
{-@ ccc :: c:Color -> l:RBSet a -> {r:RBSet a | canBeBubbled c l r} -> RBSet a @-}
ccc :: Color -> RBSet a -> RBSet a -> RBSet a
ccc c l r = E

{-@ ddd :: {c:Color | not (tooBlack c)} -> {l:RBSet a | not normalLeaf l && prop_CT l} 
                                        -> {r:RBSet a | not normalLeaf r && prop_BB r} 
                                        -> RBSet a @-}
ddd :: Color -> RBSet a -> RBSet a -> RBSet a
ddd c l r = ccc c l r

{-@ eee :: {x:RBSet a | prop_CT x} -> {v:RBSet a | not normalLeaf v && prop_BB v} @-}
eee :: RBSet a -> RBSet a
eee t@(T _ _ x _) = T B E x E
